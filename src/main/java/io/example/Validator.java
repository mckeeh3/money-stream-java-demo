...
public record Validator<T>(List<String> reasons, Function<String, T> error) {
  ...
  public Validator<T> isLtEqZero(BigDecimal test, String reason) {
    return test.compareTo(BigDecimal.ZERO) <= 0 ? addError(reason) : this;
  }

  // create is less than or equal to zero methods for all other numeric types
  public Validator<T> isPositive(int test, String reason) {
    return test > 0 ? addError(reason) : this;
  }

  public Validator<T> isPositive(BigDecimal test, String reason) {
    return test.compareTo(BigDecimal.ZERO) <= 0 ? addError(reason) : this;
  }

  public Validator<T> isPositive(double test, String reason) {
    return test > 0 ? addError(reason) : this;
  }

  public Validator<T> isPositive(long test, String reason) {
    return test > 0 ? addError(reason) : this;
  }

  public Validator<T> isPositive(short test, String reason) {
    return test > 0 ? addError(reason) : this;
  }

  public Validator<T> isPositive(float test, String reason) {
    return test > 0 ? addError(reason) : this;
  }
